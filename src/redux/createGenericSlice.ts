import { AsyncThunk, Dispatch, Draft, PayloadAction, createSlice } from "@reduxjs/toolkit";
...
function createGenericSlice<T, Props>({ name, fetchThunk }: CreateGenericSliceType<T, Props>) {
  ...
  const genericSlice = createSlice({
    ...
    extraReducers: (builder) => {
      ...
      builder.addCase(fetchThunk.fulfilled, (state, action: PayloadAction<T | null>) => {
        state.loading = false;
        state.data = action.payload as Draft<T>;
      });
      builder.addCase(fetchThunk.rejected, (state, action: PayloadAction<unknown>) => {
        state.loading = false;
